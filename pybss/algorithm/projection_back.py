...
def projection_back(Y, reference):
    ...
    if n_dims == 2:
        ...
        # X: (1, n_bins, n_frames)
        X = reference[np.newaxis, :, :].transpose(
            1, 0, 2
        )  # (n_bins, n_channels, n_frames)
        Y = Y.transpose(1, 0, 2)  # (n_bins, n_sources, n_frames)
        Y_Hermite = Y.transpose(0, 2, 1).conj()  # (n_bins, n_frames, n_sources)
        YY_Hermite_inverse = np.linalg.inv(
            Y @ Y_Hermite
        )  # (n_bins, n_sources, n_sources)
        ...
        raise ValueError(
            "reference.ndim is expected 2 or 3, but given {}.".format(n_dims)
        )
